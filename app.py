...
try:
    ...
    # Usamos el modelo que indicaste
...
def ejecutar_comando_charly(texto, df):
    ...
    for uid, field, val in updates:
        ...
        st.toast(f"✅ Supabase actualizado: {uid}")
        st.cache_data.clear()
...
if "messages" not in st.session_state:
    st.session_state.messages = [{"role": "assistant", "content": "Hola Marcelo, soy Charly. Tengo acceso total a la flota RBPE. ¿En qué buque nos enfocamos hoy?"}]
...
if prompt := st.chat_input("Dime algo, Marcelo..."):
    st.session_state.messages.append({"role": "user", "content": prompt})
    with st.chat_message("user"): st.markdown(prompt)
    
    with st.chat_message("assistant"):
        contexto = f"Eres Charly, analista naval. Año actual: 2026. Datos: {df_actual.to_csv(index=False)}. Comandos: [FICHA: ID], [UPDATE: ID, Campo, Valor]. Gráficos: ```python ... ``` con 'fig'."
        try:
